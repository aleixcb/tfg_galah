...
class galah_dr3_rc(object):
    def __init__(self, abundances=True):
        ...
        * ../data/GALAH_DR3_joint_allstar_v2_redclump_flagstatistics.txt
        we exclude the elements Li, C,Rb, Sr, Mo, Ru, Sm
        """
        hdu = pyfits.open('../data/GALAH_DR3_joint_allstar_v2_redclump.fits', names=True)
        data = hdu[1].data
        if abundances:
    ...
    def get_ndimspace(self, age=False, kin=False, feh=True, norm="stdev"):
        """
        ...
        * Fe, O, Na, Mg, Al, Si, K, Ca, Sc, Ti, V, Cr, Mn, Co, Ni, Cu, Zn, Y, Zr, Ba, La, Ce, Nd, Eu
        ...
        if not age:
            X = X[:,:-1]; Xerr = Xerr[:-1]
        if kin:
            X    = np.append(X, np.vstack((data['vR_Rzphi'],data['vT_Rzphi'],data['vZ_Rzphi'])).T, axis=1)
